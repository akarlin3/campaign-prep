// Treasure Hoard tables — ORIGINAL CONTENT.
//
// Source / licensing: SRD 5.1 (CC-BY-4.0) does NOT include the DMG treasure
// hoard tables verbatim. This file is original work authored for this
// project. The structure (CR-tiered coin scaling, gem value bands, art
// objects, magic item bands by rarity) follows conventions widely shared
// across CC-licensed and ORC-licensed tabletop SRDs, but every entry, every
// dice expression, and every multiplier is original — nothing was copied
// from any non-SRD source.
//
// Tier definitions:
//   "0-4"   — apprentice tier (party levels 1–4)
//   "5-10"  — journeyman tier (5–10)
//   "11-16" — master tier (11–16)
//   "17+"   — legendary tier (17–20)
//
// Hoard types:
//   "Individual Treasure" — coins only, smaller scale
//   "Treasure Hoard"      — coins + gems/art + magic items, larger scale

import type { ItemCategory, ItemRarity } from '../types';

export type CrTier = '0-4' | '5-10' | '11-16' | '17+';
export type HoardType = 'Individual Treasure' | 'Treasure Hoard';

// Coin yields: each tier defines a multiplier and dice expression per coin
// type. Expressed as { dice: 'NdS', multiplier: number }. Multiplier scales
// the final sum.
export type CoinFormula = { dice: string; multiplier: number };

export const COIN_TABLES: Record<HoardType, Record<CrTier, Partial<Record<'cp' | 'sp' | 'ep' | 'gp' | 'pp', CoinFormula>>>> = {
  'Individual Treasure': {
    '0-4':   { cp: { dice: '5d6', multiplier: 1 }, sp: { dice: '4d6', multiplier: 1 }, gp: { dice: '3d6', multiplier: 1 } },
    '5-10':  { sp: { dice: '4d6', multiplier: 10 }, gp: { dice: '2d6', multiplier: 10 }, pp: { dice: '1d4', multiplier: 1 } },
    '11-16': { gp: { dice: '4d6', multiplier: 10 }, pp: { dice: '2d6', multiplier: 1 } },
    '17+':   { gp: { dice: '6d6', multiplier: 100 }, pp: { dice: '4d6', multiplier: 10 } },
  },
  'Treasure Hoard': {
    '0-4':   { cp: { dice: '6d6', multiplier: 1 }, sp: { dice: '3d6', multiplier: 10 }, gp: { dice: '2d6', multiplier: 10 } },
    '5-10':  { cp: { dice: '2d6', multiplier: 100 }, sp: { dice: '2d6', multiplier: 100 }, gp: { dice: '6d6', multiplier: 10 }, pp: { dice: '3d6', multiplier: 1 } },
    '11-16': { gp: { dice: '4d6', multiplier: 100 }, pp: { dice: '5d6', multiplier: 10 } },
    '17+':   { gp: { dice: '12d6', multiplier: 100 }, pp: { dice: '8d6', multiplier: 10 } },
  },
};

// Gem and art tables (originals — named generically so users feel free to
// rename / theme them per campaign).
export const GEM_BANDS: Record<CrTier, { value: number; names: string[] }[]> = {
  '0-4': [
    { value: 10, names: ['azurite chip', 'banded agate', 'blue quartz', 'eye agate', 'hematite cabochon', 'lapis lazuli bead', 'malachite shard', 'moss agate', 'obsidian flake', 'tiger eye', 'rose quartz pebble', 'polished ironstone', 'raw turquoise', 'pyrite nodule', 'milky quartz', 'uncut malachite', 'fluorite crystal', 'snowflake obsidian', 'cloudy lapis', 'green agate'] },
    { value: 50, names: ['bloodstone', 'carnelian', 'chalcedony bead', 'chrysoprase', 'citrine', 'jasper', 'moonstone fragment', 'onyx button', 'rose quartz cabochon', 'zircon', 'sunstone', 'sardonyx chip', 'clear quartz', 'smoky quartz', 'amethyst chip', 'star rose quartz', 'aventurine bead', 'polished jadeite', 'blue lace agate', 'rhodonite bead'] },
  ],
  '5-10': [
    { value: 50, names: ['carnelian heart', 'jasper inlay', 'moonstone disc', 'onyx tile', 'zircon pendant', 'rose quartz egg', 'bloodstone cameo', 'citrine teardrop', 'chalcedony seal', 'chrysoprase band', 'sunstone disc', 'aventurine token'] },
    { value: 100, names: ['alexandrite', 'aquamarine bead', 'black pearl', 'green tourmaline', 'peridot', 'topaz cabochon', 'violet garnet', 'pink tourmaline', 'blue spinel', 'white pearl', 'golden topaz', 'red garnet', 'tsavorite garnet', 'jade cabochon'] },
    { value: 500, names: ['amethyst geode shard', 'chrysoberyl', 'coral cluster', 'jade figure', 'jet ring', 'spinel', 'flawless aquamarine', 'black opal fragment', 'golden pearl', 'fire agate', 'star diopside', 'rubellite tourmaline'] },
  ],
  '11-16': [
    { value: 100, names: ['amethyst heart', 'jet brooch', 'peridot ring', 'spinel chip', 'aquamarine teardrop', 'black pearl stud', 'violet garnet ring', 'topaz prism'] },
    { value: 500, names: ['amber bead', 'amethyst geode', 'chrysoberyl pendant', 'coral fan', 'garnet rosette', 'jade tablet', 'pearl strand', 'tourmaline', 'black opal cabochon', 'golden topaz pendant', 'blue spinel ring', 'tsavorite teardrop', 'white pearl choker', 'pink tourmaline heart', 'flawless peridot', 'star ruby fragment'] },
    { value: 1000, names: ['black opal', 'blue sapphire', 'emerald cabochon', 'fire opal', 'opal egg', 'sapphire', 'star sapphire', 'flawless ruby', 'white diamond chip', 'canary diamond chip', 'royal blue sapphire', 'emerald prism'] },
  ],
  '17+': [
    { value: 500, names: ['amber ring', 'jade pendant', 'pearl rope', 'tourmaline tile', 'coral crown', 'garnet diadem fragment', 'chrysoberyl seal', 'amethyst cluster'] },
    { value: 1000, names: ['blue sapphire', 'emerald bead', 'opal disc', 'sapphire shard', 'star sapphire cabochon', 'fire opal heart', 'flawless black opal', 'ruby bead'] },
    { value: 5000, names: ['black sapphire', 'diamond', 'jacinth', 'ruby of the deeps', 'flawless diamond', 'star ruby', 'emerald of the first forest', 'blue diamond'] },
  ],
};

export const ART_BANDS: Record<CrTier, { value: number; names: string[] }[]> = {
  '0-4': [
    { value: 25, names: ['silver ewer', 'carved bone scrollcase', 'small gold locket', 'silvered drinking horn', 'enameled brass mirror', 'inlaid wooden box', 'polished copper plate', 'carved soapstone figurine', 'bronze incense burner', 'silver-plated scabbard', 'embroidered silk handkerchief', 'copper torque', 'painted ceramic urn', 'glass decanter'] },
  ],
  '5-10': [
    { value: 250, names: ['gold mask of a forgotten hero', 'jeweled clasp', 'silver chalice with ruby inlay', 'enameled hawk statuette', 'ivory diptych', 'silk tapestry of a stag hunt', 'gilded reliquary', 'obsidian puzzle box', 'gold-rimmed drinking horn', 'velvet cloak with silver thread', 'carved alabaster bust', 'brass astrolabe', 'silver holy symbol', 'jade-handled dagger'] },
    { value: 750, names: ['gold ceremonial dagger', 'crystal sphere on bronze tripod', 'silver crown of a minor lord', 'pearl-tipped fan', 'gilded carriage clock', 'platinum-plated gauntlet', 'emerald-eyed ivory cat', 'silk ceremonial robes'] },
  ],
  '11-16': [
    { value: 750, names: ['silver coronet of a minor princess', 'gold-leafed atlas', 'ivory throne fragment', 'jeweled letter opener', 'platinum drinking cup', 'carved jade comb', 'obsidian mirror with gold frame', 'sapphire-studded silver belt'] },
    { value: 2500, names: ['gold dragon statuette', 'platinum ring of a slain king', 'silver harp with garnet strings', 'jeweled funerary mask', 'diamond-encrusted chalice', 'mithral tiara', 'tapestry woven with gold thread', 'ancient ivory spellbook cover'] },
  ],
  '17+': [
    { value: 2500, names: ['gold throne plate', 'platinum coronet', 'silver effigy of a saint', 'jeweled war horn', 'ruby-hilted longsword scabbard', 'gilded wyvern skull', 'platinum astrolabe', 'crown of woven mithral'] },
    { value: 7500, names: ['platinum statuette of a god', 'ruby-set crown of a dead empire', 'gilded altarpiece studded with diamonds', 'jade dragon larger than a child', 'obsidian throne fragment', 'diamond-studded platinum scepter', 'tapestry of a celestial court', 'emerald-encrusted golden idol'] },
  ],
};

// Magic items per rarity tier. ORIGINAL names with light category tagging.
export type MagicItemEntry = { name: string; rarity: ItemRarity; category: ItemCategory; note?: string };

export const MAGIC_ITEMS: Record<Exclude<ItemRarity, 'mundane'>, MagicItemEntry[]> = {
  common: [
    { name: 'Candle of the Quiet Hour', rarity: 'common', category: 'wondrous', note: 'A wax taper that burns blue and silences soft sounds within ten feet.' },
    { name: 'Compass of Lost Things', rarity: 'common', category: 'wondrous', note: 'Needle points to an object the bearer named that morning, if within a mile.' },
    { name: 'Mug of the Drowning Sip', rarity: 'common', category: 'wondrous', note: 'Fills with clean water each dawn.' },
    { name: 'Pin of the Mended Cloak', rarity: 'common', category: 'wondrous', note: 'Repairs one tear or scuff per day in cloth or leather.' },
    { name: 'Lantern of Memory', rarity: 'common', category: 'wondrous', note: 'Burns without fuel; shows shadows of those who held it last.' },
    { name: 'Scroll of Polite Greeting', rarity: 'common', category: 'scroll', note: 'Casts the spell of formal greeting in twelve known dialects.' },
    { name: 'Bracelet of Soft Landing', rarity: 'common', category: 'wondrous', note: 'Once per day, halves falling damage when the wearer braces.' },
    { name: 'Pipe of Counting Smoke', rarity: 'common', category: 'wondrous', note: 'Smoke rings form numerals that count up to a hundred.' },
    { name: 'Quill of the Bound Promise', rarity: 'common', category: 'wondrous', note: 'A signature made with this quill cannot be later denied by the signer.' },
    { name: 'Ribbon of Strait Truth', rarity: 'common', category: 'wondrous', note: 'When tied around a wrist, the wearer cannot speak in untruths without effort.' },
    { name: 'Coin of the Final Wager', rarity: 'common', category: 'wondrous', note: 'Always lands on the face called, but only once per person.' },
    { name: 'Spoon of Sweet Porridge', rarity: 'common', category: 'wondrous', note: 'Makes any bland meal taste of honey and cinnamon.' },
    { name: 'Key of the Forgotten Drawer', rarity: 'common', category: 'wondrous', note: 'Opens any non-magical lock that no one has used in a decade.' },
    { name: 'Locket of the Distant Friend', rarity: 'common', category: 'wondrous', note: 'Warms slightly when a chosen friend is thinking of the wearer.' },
    { name: 'Flask of Endless Tea', rarity: 'common', category: 'wondrous', note: 'Produces one cup of hot herbal tea each short rest.' },
    { name: 'Dust of the Tidy Hearth', rarity: 'common', category: 'wondrous', note: 'When sprinkled, cleans a 10-foot square area perfectly.' },
    { name: 'Bell of the Waking Watch', rarity: 'common', category: 'wondrous', note: 'Rings silently to the bearer if someone approaches within 30 feet.' },
    { name: 'Parchment of the Long Memory', rarity: 'common', category: 'wondrous', note: 'Records anything spoken within 5 feet when unrolled.' },
    { name: 'Boots of the Clean Step', rarity: 'common', category: 'wondrous', note: 'Never gather mud or dirt, regardless of the terrain.' },
    { name: 'Chalk of the Glowing Mark', rarity: 'common', category: 'wondrous', note: 'Marks drawn with this chalk glow faintly in the dark for a week.' },
  ],
  uncommon: [
    { name: 'Boots of the Heron Step', rarity: 'uncommon', category: 'wondrous', note: 'Long strides across rooftops or shallow water leave no print for a heartbeat.' },
    { name: 'Cloak of the Hollow Wind', rarity: 'uncommon', category: 'wondrous', note: 'Once per dawn, the wearer slips one breath out of view.' },
    { name: 'Ring of the Lender\'s Tongue', rarity: 'uncommon', category: 'ring', note: 'Allows comprehension of one chosen language for an hour each day.' },
    { name: 'Wand of Borrowed Sparks', rarity: 'uncommon', category: 'wand', note: 'Channels small lightning to inanimate targets — five charges, regains 1d4 at dawn.' },
    { name: 'Potion of the Even Pulse', rarity: 'uncommon', category: 'potion', note: 'Steadies a panicked or charmed mind for ten minutes.' },
    { name: 'Amulet of the Reading Mirror', rarity: 'uncommon', category: 'wondrous', note: 'Shows the reflection of any nearby written word, even if hidden.' },
    { name: 'Buckler of the Patient Defense', rarity: 'uncommon', category: 'armor', note: 'Once per short rest, raises to block a single hostile spell aimed at the bearer.' },
    { name: 'Dagger of the Quiet Step', rarity: 'uncommon', category: 'weapon', note: 'Strikes that miss leave no sound.' },
    { name: 'Censer of the Bound Spirit', rarity: 'uncommon', category: 'wondrous', note: 'Smoke from this censer reveals invisible passages in a hallway for one minute.' },
    { name: 'Spectacles of the Reading Sun', rarity: 'uncommon', category: 'wondrous', note: 'Allow the wearer to read in any light, including starlight.' },
    { name: 'Vial of the Last Word', rarity: 'uncommon', category: 'potion', note: 'Drunk after a death, lets the corpse speak one short answer.' },
    { name: 'Tome of the Indexed Wilderness', rarity: 'uncommon', category: 'wondrous', note: 'Self-updates with the bearer\'s recent travels; +2 to wilderness recall.' },
    { name: 'Bracers of the Iron Grip', rarity: 'uncommon', category: 'wondrous', note: 'Grants advantage on climbing and prevents being disarmed.' },
    { name: 'Lantern of the Ghostly Flame', rarity: 'uncommon', category: 'wondrous', note: 'Burns with a cold blue fire that reveals ethereal entities within 30 feet.' },
    { name: 'Rope of the Snake\'s Coil', rarity: 'uncommon', category: 'wondrous', note: 'A 50-foot silken rope that ties and unties itself on command.' },
    { name: 'Shield of the Bounding Stag', rarity: 'uncommon', category: 'armor', note: 'Grants the wielder a sudden burst of speed once per combat encounter.' },
    { name: 'Ring of the Water strider', rarity: 'uncommon', category: 'ring', note: 'Allows the wearer to walk on calm water for up to an hour a day.' },
    { name: 'Cloak of the Autumn Leaf', rarity: 'uncommon', category: 'wondrous', note: 'Blend perfectly into natural forest surroundings if standing still.' },
    { name: 'Gauntlets of the Stonebreaker', rarity: 'uncommon', category: 'wondrous', note: 'Double damage against objects and structures.' },
    { name: 'Wand of the Whisperer', rarity: 'uncommon', category: 'wand', note: 'Carries a spoken message on the wind to a target within a mile.' },
    { name: 'Brooch of the Unseen Eye', rarity: 'uncommon', category: 'wondrous', note: 'Vibrates warmly when the wearer is being scryed upon.' },
    { name: 'Dagger of the Returning Swallow', rarity: 'uncommon', category: 'weapon', note: 'Returns to the wielder\'s hand immediately after being thrown.' },
    { name: 'Potion of the Giant\'s Stride', rarity: 'uncommon', category: 'potion', note: 'Increases walking speed by 10 feet and jumping distance by 10 feet for an hour.' },
    { name: 'Amulet of the Still Heart', rarity: 'uncommon', category: 'wondrous', note: 'Wearer cannot be magically frightened.' },
  ],
  rare: [
    { name: 'Crown of the Listening Court', rarity: 'rare', category: 'wondrous', note: 'The wearer hears the truthful intent behind any oath spoken in their presence.' },
    { name: 'Greatsword of the Riverbed', rarity: 'rare', category: 'weapon', note: 'Drinks blood spilled on it; the blade darkens for each life it has cost.' },
    { name: 'Mantle of the Tidewatcher', rarity: 'rare', category: 'wondrous', note: 'Grants water-breathing in salt water; the wearer dreams of tides they have never seen.' },
    { name: 'Ring of the Borrowed Hour', rarity: 'rare', category: 'ring', note: 'Once per dawn, the wearer may take a sixty-second action as if no time had passed.' },
    { name: 'Staff of the Quiet Wizard', rarity: 'rare', category: 'staff', note: 'Casts known spells without verbal components, but charges noticeably with each use.' },
    { name: 'Tome of the Falling Page', rarity: 'rare', category: 'wondrous', note: 'A new page appears each midnight; the words foretell a small event of the coming day.' },
    { name: 'Mask of the Borrowed Face', rarity: 'rare', category: 'wondrous', note: 'Shows the wearer the face of any one person they have spoken with this week.' },
    { name: 'Bracers of the Counting Wind', rarity: 'rare', category: 'wondrous', note: 'Once a day, deflect a single projectile by raising the bearer\'s forearm.' },
    { name: 'Lantern of the Pilgrim\'s Road', rarity: 'rare', category: 'wondrous', note: 'Lights only when carried toward a destination the bearer has named aloud.' },
    { name: 'Shield of the Saved Sigil', rarity: 'rare', category: 'armor', note: 'Once per day, absorbs a single spell aimed at a comrade within ten feet of the bearer.' },
    { name: 'Cloak of the Shadowed Wing', rarity: 'rare', category: 'wondrous', note: 'Allows the wearer to glide from a height, becoming invisible in dim light.' },
    { name: 'Amulet of the Sun\'s Promise', rarity: 'rare', category: 'wondrous', note: 'Stores sunlight during the day and can release it as a blinding flash.' },
    { name: 'Ring of the Iron Will', rarity: 'rare', category: 'ring', note: 'Grants advantage on all saves against mind-altering magic.' },
    { name: 'Wand of the Frost-Biter', rarity: 'rare', category: 'wand', note: 'Fires shards of ice; targets must save or be slowed.' },
    { name: 'Longbow of the Piercing Gale', rarity: 'rare', category: 'weapon', note: 'Arrows fired from this bow ignore wind resistance and magical barriers.' },
    { name: 'Armor of the Earth-Bringer', rarity: 'rare', category: 'armor', note: 'Half-plate that allows the wearer to meld into stone once per day.' },
    { name: 'Staff of the Sprouting Vine', rarity: 'rare', category: 'staff', note: 'Can animate plants to grapple foes or grow bridges of thick vines.' },
    { name: 'Boots of the Cloud-Walker', rarity: 'rare', category: 'wondrous', note: 'Allows the wearer to walk on air for up to ten minutes a day.' },
    { name: 'Chalice of the Purified Soul', rarity: 'rare', category: 'wondrous', note: 'Any liquid drunk from it cures diseases and neutralizes poisons.' },
    { name: 'Dagger of the Mind-Thief', rarity: 'rare', category: 'weapon', note: 'On a critical hit, steals one prepared spell from the target\'s memory.' },
  ],
  'very rare': [
    { name: 'Bow of the Last Stand', rarity: 'very rare', category: 'weapon', note: 'On the day of the bearer\'s death, fires each arrow with unerring aim.' },
    { name: 'Censer of Forgotten Names', rarity: 'very rare', category: 'wondrous', note: 'Smoke spells out the true name of one nearby creature once per dawn.' },
    { name: 'Helm of the Storm-King', rarity: 'very rare', category: 'wondrous', note: 'Hair stands on end during a storm; bearer may call lightning down once per moon.' },
    { name: 'Ledger of the Sealed Debt', rarity: 'very rare', category: 'wondrous', note: 'Anyone whose name is written within feels compelled to repay a debt they have forgotten.' },
    { name: 'Robe of the Stitched Hour', rarity: 'very rare', category: 'wondrous', note: 'Each thread is a minute. The wearer may unstitch one thread to undo their last action.' },
    { name: 'Sword of the Bound Promise', rarity: 'very rare', category: 'weapon', note: 'Will not strike a creature to whom its bearer has made an oath, until the oath ends.' },
    { name: 'Ring of the Painted Door', rarity: 'very rare', category: 'ring', note: 'Once per dawn, draws a door on any wall through which the bearer may pass for one minute.' },
    { name: 'Tome of the Mended Throne', rarity: 'very rare', category: 'wondrous', note: 'When held by a dying ruler, names the next ruler in the bearer\'s own handwriting.' },
    { name: 'Mantle of the Star-Weaver', rarity: 'very rare', category: 'wondrous', note: 'Woven from night sky; the wearer can step between shadows cast by starlight.' },
    { name: 'Staff of the Earthquake', rarity: 'very rare', category: 'staff', note: 'Striking the ground causes a localized tremor that knocks enemies prone.' },
    { name: 'Shield of the Gorgon\'s Glare', rarity: 'very rare', category: 'armor', note: 'Once per week, reflects a petrifying gaze or casts flesh to stone.' },
    { name: 'Amulet of the Phoenix Heart', rarity: 'very rare', category: 'wondrous', note: 'If the wearer drops to 0 hit points, they explode in fire and drop to 1 hit point instead.' },
    { name: 'Gauntlets of the Titan\'s Grasp', rarity: 'very rare', category: 'wondrous', note: 'Grants the wearer the strength to tear down fortress gates with bare hands.' },
    { name: 'Spear of the Comet\'s Tail', rarity: 'very rare', category: 'weapon', note: 'When thrown, turns into a streak of pure force that pierces multiple targets.' },
    { name: 'Cloak of the Void-Walker', rarity: 'very rare', category: 'wondrous', note: 'Allows the wearer to become intangible, passing through solid matter for one minute.' },
    { name: 'Ring of the Sovereign\'s Command', rarity: 'very rare', category: 'ring', note: 'Once per day, issue an undeniable command to a creature of lesser will.' },
  ],
  legendary: [
    { name: 'Crown of the Drowned Empire', rarity: 'legendary', category: 'wondrous', note: 'Bestows the memory of every drowned monarch upon the wearer; salt water leaks from their eyes when worn.' },
    { name: 'Hourglass of the Unwritten Day', rarity: 'legendary', category: 'wondrous', note: 'When inverted, gives the bearer a single day no one else will remember.' },
    { name: 'Sceptre of the Last Verdict', rarity: 'legendary', category: 'wondrous', note: 'A verdict spoken while holding this sceptre becomes binding on any creature within sight.' },
    { name: 'Sword of the Cleaved Mountain', rarity: 'legendary', category: 'weapon', note: 'Once per moon, parts solid stone in front of the wielder to a depth of ten feet.' },
    { name: 'Tome of the Final Page', rarity: 'legendary', category: 'wondrous', note: 'The last entry is always the next true event in the bearer\'s life — written in their own hand.' },
    { name: 'Aegis of the Fallen Star', rarity: 'legendary', category: 'armor', note: 'A shield forged from a meteorite; grants immunity to fire and reflects breath weapons.' },
    { name: 'Bow of the Sun-Chaser', rarity: 'legendary', category: 'weapon', note: 'Fires arrows of pure sunlight that banish darkness and turn undead to ash.' },
    { name: 'Mantle of the World-Tree', rarity: 'legendary', category: 'wondrous', note: 'Connects the wearer to the roots of the earth, granting limitless endurance.' },
    { name: 'Ring of the Fate-Weaver', rarity: 'legendary', category: 'ring', note: 'Allows the wearer to completely reroll any one event that occurred in the last hour, once per decade.' },
    { name: 'Staff of the Primordial Storm', rarity: 'legendary', category: 'staff', note: 'Commands weather on a continental scale; the bearer\'s eyes crackle with permanent lightning.' },
  ],
};

// Magic-item bands per CR tier and hoard type — controls how many items, what
// rarity, and the chance any items appear at all.
export type MagicItemBand = {
  chance: number;      // 0–1, chance hoard contains any magic items
  count: { dice: string; min?: number; max?: number };
  rarityWeights: { [K in Exclude<ItemRarity, 'mundane'>]?: number };
};

export const MAGIC_ITEM_BANDS: Record<HoardType, Record<CrTier, MagicItemBand>> = {
  'Individual Treasure': {
    '0-4':   { chance: 0.0, count: { dice: '0d1' }, rarityWeights: {} },
    '5-10':  { chance: 0.0, count: { dice: '0d1' }, rarityWeights: {} },
    '11-16': { chance: 0.0, count: { dice: '0d1' }, rarityWeights: {} },
    '17+':   { chance: 0.0, count: { dice: '0d1' }, rarityWeights: {} },
  },
  'Treasure Hoard': {
    '0-4':   { chance: 0.45, count: { dice: '1d4', min: 1, max: 4 }, rarityWeights: { common: 8, uncommon: 3 } },
    '5-10':  { chance: 0.70, count: { dice: '1d4+1', min: 2, max: 5 }, rarityWeights: { common: 4, uncommon: 8, rare: 2 } },
    '11-16': { chance: 0.85, count: { dice: '1d6+1', min: 2, max: 7 }, rarityWeights: { uncommon: 3, rare: 7, 'very rare': 3 } },
    '17+':   { chance: 0.95, count: { dice: '2d4', min: 2, max: 8 }, rarityWeights: { rare: 4, 'very rare': 7, legendary: 2 } },
  },
};

// Gem/art count bands — uniform 0..N picks per band per tier per hoard type.
export const GEM_COUNT: Record<HoardType, Record<CrTier, { dice: string }>> = {
  'Individual Treasure': { '0-4': { dice: '0d1' }, '5-10': { dice: '0d1' }, '11-16': { dice: '0d1' }, '17+': { dice: '0d1' } },
  'Treasure Hoard':      { '0-4': { dice: '1d4' }, '5-10': { dice: '1d6' }, '11-16': { dice: '1d6+1' }, '17+': { dice: '2d4' } },
};

export const ART_COUNT: Record<HoardType, Record<CrTier, { dice: string }>> = {
  'Individual Treasure': { '0-4': { dice: '0d1' }, '5-10': { dice: '0d1' }, '11-16': { dice: '0d1' }, '17+': { dice: '0d1' } },
  'Treasure Hoard':      { '0-4': { dice: '1d4' }, '5-10': { dice: '1d4' }, '11-16': { dice: '1d6' }, '17+': { dice: '1d6+1' } },
};
